...
```
python run.py --device 0 --checkpoint_dir my_ckpt --task binary --batch_size 128 --lr 1e-4 --valid_every 10
```
```
...
- Inference만 수행하기
```
python run.py --device 0 --do_eval --task binary --batch_size 16 --from_checkpoint <ckpt_path> 
```
```
dropping 1 samples due to nan values...
dropping 0 samples due to nan values...
